...
def get_products():
	sesion = request.headers['Sesion']
	cur=mysql.connection.cursor()
	estado_sesion = cur.execute('SELECT * FROM token where token_cod= %s', [sesion])
	cur.fetchall()
	if estado_sesion==1:
		...
		return "Sesion expirada, inicie sesion"
...
def login():
	...
	if estado_query==1:	
		...
		item={
			...
			'password':respuesta[0][2]}
		token = uuid.uuid4();
		cur.execute('INSERT INTO token (token_cod, correo) VALUES (%s, %s)', (token, correo))
		cur.connection.commit()
		return  jsonify(Sesion=token)
	else:
...
def logout():
	...
	cur.fetchall()
	if estado_sesion==1:
		cur.execute('DELETE FROM token WHERE token_cod = %s', [sesion])
		cur.connection.commit()
		return "Done"
	else:
		return "Sesion expirada, inicie sesion"
